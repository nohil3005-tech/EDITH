...
INSERT OR IGNORE INTO user_platforms (id, user_id, platform_name, platform_url, icon_url, status, last_synced, notifications_count, messages_count, is_active)
VALUES
  ('platform-upwork', '${DEFAULT_USER_ID}', 'Upwork', 'https://upwork.com', '🟢', 'Connected', 'Just now', 3, 2, 1),
  ('platform-fiverr', '${DEFAULT_USER_ID}', 'Fiverr', 'https://fiverr.com', '🟢', 'Connected', '5m ago', 1, 0, 1);

-- Seed default freelance jobs
INSERT OR IGNORE INTO freelance_jobs (id, user_id, source_platform, external_id, title, description, budget_min, budget_max, client_rating, tags, ai_score, ai_insights, status)
VALUES
  ('job-001', '${DEFAULT_USER_ID}', 'upwork', 'mock-9', 'AI Consulting and RAG Pipeline Design', 'Consult on setting up an LLM-based RAG pipeline for corporate document search. Architect document indexing, vector stores, and prompt templates.', 2000, 5000, 5.0, '["AI Consulting","Python","LLM"]', 92, '{"matchScore":92,"strengths":["High budget","AI stack matching"],"concerns":[],"suggestedBid":3500,"estimatedDays":10,"summary":"Perfect fit"}', 'new'),
  ('job-002', '${DEFAULT_USER_ID}', 'upwork', 'mock-10', 'Shopify Store Builder & Product Importer', 'Design and build a professional Shopify drop-shipping store from scratch. Install plugins, import trending products, configure checkout, and design a custom banner.', 500, 1500, 4.8, '["Store Builder","Shopify","Web Dev"]', 85, '{"matchScore":85,"strengths":["Matches e-commerce skills"],"concerns":[],"suggestedBid":1200,"estimatedDays":7,"summary":"Good fit"}', 'new'),
  ('job-003', '${DEFAULT_USER_ID}', 'fiverr', 'mock-11', 'Blog Post Wellness Campaign', 'Write three 800-word blog posts on the benefits of daily meditation and yoga.', 150, 450, 4.9, '["Content","Writing","Blog"]', 88, '{"matchScore":88,"strengths":["Writing domain match"],"concerns":[],"suggestedBid":300,"estimatedDays":4,"summary":"Engaging writing"}', 'new'),
  ('job-004', '${DEFAULT_USER_ID}', 'freelancer', 'mock-12', 'Technical Documentation rest API manual', 'Develop API documentation rest guide.', 400, 1000, 4.9, '["Content","Developer","Writing"]', 90, '{"matchScore":90,"strengths":["Technical docs"],"concerns":[],"suggestedBid":800,"estimatedDays":6,"summary":"API specifications"}', 'new');

-- Seed proposals
INSERT OR IGNORE INTO proposals (id, user_id, job_id, draft_text, final_text, bid_amount, delivery_days, status)
VALUES
...
INSERT OR IGNORE INTO active_freelance_jobs (id, user_id, job_id, proposal_id, column_name, subtasks)
VALUES
